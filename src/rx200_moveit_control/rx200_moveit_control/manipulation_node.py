...
class MoveItEEClient(Node):
    ...
    def pick_place_cubes(self, cubes_data: dict, color_order: list, place_position: list, stacking_height=0.015):
        ...
        for key, value in cubes_data.items():
            ...
            # lift up to avoid colliding with other cubes
            self.send_pose(x_pick, y_pick, z_hover, pitch=pitch)
            while not self.motion_done:
                rclpy.spin_once(self, timeout_sec=0.1)
...
def main():
    ...
    cubes_data = {
        "red": [0.25, 0.15, 0.085],
        "blue": [0.25, -0.15, 0.085],
        "yellow": [0.2, -0.2, 0.085]
    }
